        ...
        measurements: measurementRows
          ...
          .map(row => {
            ...
            return {
              ...
              unit: 'ft' as const,
              confidence: row.source === 'manual' ? 1 : Number(row.calibrated_confidence ?? rawConfidence),
              rawConfidence,
              calibratedConfidence: row.calibrated_confidence ?? undefined,
              source: row.source === 'manual' ? 'manual' as const : 'estimated' as const,
              uncertaintyLow: row.source === 'manual' ? undefined : row.uncertainty_low ?? undefined,
              uncertaintyHigh: row.source === 'manual' ? undefined : row.uncertainty_high ?? undefined,
